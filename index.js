...
bot.callbackQuery('2', async (ctx) => {
    await ctx.answerCallbackQuery();
    const EGEtwoKeyboard = new InlineKeyboard().text('ЕГЭ в основной период', '21').row().text('ЕГЭ в досрочный период', '22').row().text('ЕГЭ в дополнительный период', '23').row().text('ЕГЭ в резервный период (для кого)', '24').row().text('Назад', 'back');
    try {
        // Редактируем предыдущее сообщение, заменяя его новым текстом и клавиатурой
...
//2.4 ЕГЭ В РЕЗЕРВНЫЙ ПЕРИОД (для кого)
...
//2.3 ЕГЭ В ДОПОЛНИТЕЛЬНЫЙ ПЕРИОД
...
bot.callbackQuery('4', async (ctx) => {
    await ctx.answerCallbackQuery();
    const EGEfourKeyboard = new InlineKeyboard().text('Необходимые документы и принадлежности', '41').row().text('Разрешенные средства обучения и воспитания', '42').row().text('Что нельзя брать с собой на экзамен', '43').row().text('Правила поведения на экзамене и последствия', '44').row().text('Назад', 'back');
    try {
        // Редактируем предыдущее сообщение, заменяя его новым текстом и клавиатурой
...
bot.callbackQuery('41', async (ctx) => {
    ...
    try {
        // Редактируем предыдущее сообщение, заменяя его новым текстом и клавиатурой
        const sentMessage = await ctx.editMessageText('📍 ЕГЭ \n\n➡️ _Правила и требования на экзамене_ \n➡️ _Необходимые документы и принадлежности_ \n\n✅ Во время экзамена на рабочем столе сдающего помимо экзаменационных материалов находятся: \n\n1\\)гелевая или капиллярная ручка с чернилами черного цвета; \n\n2\\)документ, удостоверяющий личность; \n\n3\\)средства обучения и воспитания \\(Смотрите раздел «Разрешенные средства обучения и воспитания», будут в отдельном разделе\\); \n\n4\\)лекарства \\(при необходимости\\); \n\n5\\) черновики, выданные в пункте проведения экзамена\\. \n\nВсе другие личные вещи участники должны оставить в специально отведенном месте перед входом в пункт\\.', {
            //reply_markup: essayKeyboard
            reply_markup: InlineMenuKeyboard, parse_mode: 'MarkdownV2', disable_web_page_preview: true
        });
        previousMessage = {
...
//4.2 ЧТО МОЖНО БРАТЬ С СОБОЙ НА ЭКЗАМЕН В АУДИТОРИЮ (Разрешенные средства обучения и воспитания)
...
bot.callbackQuery('52', async (ctx) => {
    ...
    try {
        // Редактируем предыдущее сообщение, заменяя его новым текстом и клавиатурой
        const sentMessage = await ctx.editMessageText('📍 ЕГЭ \n\n➡️ _Результаты ЕГЭ_ \n➡️ _Таблицы соответствия первичных и тестовых баллов ЕГЭ_ \n\n📌 Данные за 2025 год представлены в [документе](https://obrnadzor.gov.ru/wp-content/uploads/2025/05/tabliczy_sootvetstvij_2025.pdf)', {
            //reply_markup: essayKeyboard
            reply_markup: InlineMenuKeyboard, parse_mode: 'MarkdownV2', disable_web_page_preview: true
        });
        previousMessage = {
...
bot.callbackQuery('6', async (ctx) => {
    await ctx.answerCallbackQuery();
    const EGEsixKeyboard = new InlineKeyboard().text('Что нельзя оспорить?', '61').row().text('Когда и как подавать апелляцию?', '62').row().text('Всё о пересдаче', '63').row().text('Пересдача в дополнительный период \(сентябрь\)', '64').row().text('Пересдача предметов по выбору', '65').row().text('Назад', 'back');
    try {
        // Редактируем предыдущее сообщение, заменяя его новым текстом и клавиатурой
...
//6.2 КОГДА И КАК ПОДАВАТЬ АППЕЛЯЦИЮ
...
bot.callbackQuery('9', async (ctx) => {
    await ctx.answerCallbackQuery();
    const EGEnineKeyboard = new InlineKeyboard().text('Итоговое сочинение', '91').row().text('Особенности подачи заявления', '92').row().text('Условия проведения и продолжительность экзамена', '93').row().text('Апелляции', '94').row().text('Назад', 'back');
    try {
        // Редактируем предыдущее сообщение, заменяя его новым текстом и клавиатурой
        await ctx.editMessageText('📍 ЕГЭ \n\n➡️ ЕГЭ для обучающихся с ограниченными возможностями здоровья, детей–инвалидов и инвалидов \n\nДля участников с ограниченными возможностями здоровья (ОВЗ), детей-инвалидов и инвалидов государственная итоговая аттестация (ГИА) проводится в виде государственного выпускного экзамена (ГВЭ). Также по их желанию ГИА может проводится в форме ЕГЭ. При этом допускается сочетание форм проведения (ЕГЭ и ГВЭ). \n\n❗ Результаты ГВЭ признаются только в качестве результатов ГИА-11 и являются основанием для выдачи аттестата о среднем общем образовании. \nРезультаты ГВЭ НЕ учитываются при поступлении в организации высшего образования. Поступить в организации высшего образования обучающиеся, сдававшие ГВЭ, могут по результатам вступительных испытаний, форма и перечень которых определяется образовательной организацией высшего образования самостоятельно.', {
...
//9.2 УЧАСТИЕ В ЕГЭ И ПОДАЧА ЗАЯВЛЕНИЯ (Особенности подачи заявления)
bot.callbackQuery('92', async (ctx) => {
    ...
    try {
        // Редактируем предыдущее сообщение, заменяя его новым текстом и клавиатурой
        const sentMessage = await ctx.editMessageText('📍 ЕГЭ \n\n➡️ _ЕГЭ для обучающихся с ограниченными возможностями здоровья, детей–инвалидов и инвалидов_ \n➡️ _Особенности подачи заявления_ \n\nУчастники ГИА\\-11 *с ограниченными возможностями здоровья* при подаче заявления об участии в ГИА\\-11 предъявляют *копию рекомендаций психолого\\-медико\\-педагогической комиссии*, а участники ГИА\\-11 – *дети\\-инвалиды и инвалиды* – оригинал или заверенную *копию справки, подтверждающей факт установления инвалидности, выданной федеральным государственным учреждением медико\\-социальной экспертизы* \\(справка, подтверждающая инвалидность\\), а также копию рекомендаций *психолого\\-медико\\-педагогической комиссии* для проведения экзамена в специальных условиях\\. \n\nВ заявлении указанные участники ГИА\\-11 указывают специальные условия, учитывающие состояние их здоровья, особенности психофизического развития, необходимые им при проведении экзаменов\\. \n\n❗ *Предоставление условий*, учитывающих состояние здоровья, особенности психофизического развития участников ГИА\\-11 с ограниченными возможностями здоровья, участников ГИА\\-11 детей\\-инвалидов и инвалидов, а также лиц, обучающихся по состоянию здоровья на дому, в образовательных организациях, в том числе санаторно\\-курортных, в которых проводятся необходимые лечебные, реабилитационные и оздоровительные мероприятия для нуждающихся в длительном лечении, в том числе *специальных условий*, осуществляется ТОЛЬКО ПРИ ПРЕДЪЯВЛЕНИИ ими копии *рекомендаций психолого\\-медико\\-педагогической комиссии и/или оригинала или заверенной в установленном порядке копии справки, подтверждающей инвалидность\\.*', {
            //reply_markup: essayKeyboard
            reply_markup: InlineMenuKeyboard, parse_mode: 'MarkdownV2', disable_web_page_preview: true
        });
        previousMessage = {
...
bot.callbackQuery('02', async (ctx) => {
    await ctx.answerCallbackQuery();
    const OGEtwoKeyboard = new InlineKeyboard().text('ОГЭ в основной период', '021').row().text('ОГЭ в досрочный период', '022').row().text('ОГЭ в дополнительный период', '023').row().text('ОГЭ в резервный период (для кого)', '024').row().text('Назад', 'backOge');
    try {
        // Редактируем предыдущее сообщение, заменяя его новым текстом и клавиатурой
...
bot.callbackQuery('04', async (ctx) => {
    await ctx.answerCallbackQuery();
    const OGEfourKeyboard = new InlineKeyboard().text('Необходимые документы и принадлежности', '041').row().text('Разрешенные средства обучения и воспитания', '042').row().text('Что нельзя брать с собой на экзамен', '043').row().text('Правила поведения на экзамене и последствия нарушений', '044').row().text('Назад', 'backOge');
    try {
        // Редактируем предыдущее сообщение, заменяя его новым текстом и клавиатурой
        await ctx.editMessageText('📍 ОГЭ \n\n➡️ _Правила и требования на экзамене_', {
...
//04.2 ЧТО МОЖНО БРАТЬ С СОБОЙ НА ЭКЗАМЕН (Разрешенные средства обучения и воспитания)
...
bot.callbackQuery('06', async (ctx) => {
    await ctx.answerCallbackQuery();
    const OGEsixKeyboard = new InlineKeyboard().text('Что нельзя оспорить?', '061').row().text('Когда и как подавать апелляцию?', '062').row().text('Всё о пересдаче', '063').row().text('Назад', 'backOge');
    try {
        // Редактируем предыдущее сообщение, заменяя его новым текстом и клавиатурой
...
//06.2 КОГДА И КАК ПОДАВАТЬ АПЕЛЛЯЦИЮ?
...
bot.callbackQuery('07', async (ctx) => {
    await ctx.answerCallbackQuery();
    const OGEsevenKeyboard = new InlineKeyboard().text('Условия поступления в 10 класс', '071').row().text('Условия поступления в колледж', '072').row().text('Назад', 'backOge');
    try {
        // Редактируем предыдущее сообщение, заменяя его новым текстом и клавиатурой
...
//07.2 УСЛОВИЯ ПОСТУПЛЕНИЯ В КОЛЛЕДЖ
...
bot.callbackQuery('09', async (ctx) => {
    await ctx.answerCallbackQuery();
    const OGEnineKeyboard = new InlineKeyboard().text('Итоговое собеседование по русскому языку', '091').row().text('Особенности подачи заявления', '092').row().text('Условия проведения и продолжительность экзамена', '093').row().text('Назад', 'backOge');
    try {
        // Редактируем предыдущее сообщение, заменяя его новым текстом и клавиатурой
        await ctx.editMessageText('📍 ОГЭ \n\n➡️ ОГЭ для обучающихся с ограниченными возможностями здоровья, детей–инвалидов и инвалидов \n\nДля участников с ограниченными возможностями здоровья (ОВЗ), детей-инвалидов и инвалидов государственная итоговая аттестация (ГИА) проводится в виде государственного выпускного экзамена (ГВЭ). Также по их желанию ГИА может проводится в форме ОГЭ. При этом допускается сочетание форм проведения (ОГЭ и ГВЭ). \n\n📌 Выбор предметов \n\nУчастники могут сдавать только обязательные предметы (русский язык и математика) или выбрать четыре предмета (два обязательных и два по выбору).', {
...
//09.2 ОСОБЕННОСТИ ПОДАЧИ ЗАЯВЛЕНИЯ
bot.callbackQuery('092', async (ctx) => {
    ...
    try {
        // Редактируем предыдущее сообщение, заменяя его новым текстом и клавиатурой
        const sentMessage = await ctx.editMessageText('📍 ОГЭ \n\n➡️ _ОГЭ для обучающихся с ограниченными возможностями здоровья, детей–инвалидов и инвалидов_ \n➡️ _Особенности подачи заявления_ \n\nУчастники ГИА\\-9 *с ограниченными возможностями здоровья* при подаче заявления об участии в ГИА\\-9 предъявляют: \n*копию рекомендаций психолого\\-медико\\-педагогической комиссии*\\. \n\nУчастники ГИА\\-9 – *дети\\-инвалиды и инвалиды*: \n*оригинал* или *заверенную копию справки, подтверждающей факт установления инвалидности*, выданной федеральным государственным учреждением медико\\-социальной экспертизы \\(справка, подтверждающая инвалидность\\); \n*копию рекомендаций психолого\\-медико\\-педагогической комиссии* для проведения экзамена в специальных условиях\\. \n\n📌 В заявлении указанные участники ГИА\\-9 указывают специальные условия, учитывающие состояние их здоровья, особенности психофизического развития, необходимые им при проведении экзаменов\\.', {
            //reply_markup: essayKeyboard
            reply_markup: InlineMenuKeyboard, parse_mode: 'MarkdownV2', disable_web_page_preview: true
        });
        previousMessage = {
